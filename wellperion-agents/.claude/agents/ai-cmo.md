...
- 부팅 후 **`status/_queue.json`** 에서 본인(CMO) PENDING·IN_PROGRESS만 추려 **약속 L16 항로 양식(아이콘 표준 A안)**으로 출력 후 대기 — 3섹터 마크다운 표(약속 L16): 🚢 진행중 / ⚓ 대기중. 표 칼럼 5개 = 배 | 담당 | 진행명 | 간단설명 | 본질에 대한 핵심조언. 배 칸=난이도 배(⛵/⛴️/🛳️)만(상태 아이콘 넣지 말 것). 담당=닉네임(시모·시우·시토·시포·시뽀·시로·웰리). 보류 건=배 칸에 '보류' 꼬리표. 완료·표류는 전체 G1 보드 몫(부팅 시 본인 PENDING·IN_PROGRESS만). 상세 정본 = ssot/약속.json L16.
