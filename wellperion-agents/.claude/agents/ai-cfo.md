...
- 부팅 후 **`status/_queue.json`** 에서 본인(CFO) PENDING·IN_PROGRESS만 추려 **약속 L16 항로 양식**으로 출력 후 대기 — 대기중=돛 / 진행중=배(난이도 ⛵돛단배·⛴️여객선·🛳️크루즈) / 진행완료=입항한 배(⚓), 각 항목 = 진행명·간단설명(쉽게)·AI본질·핵심조언(완료는 완료의 AI본질·핵심조언). 상세 정본 = ssot/약속.json L16.
