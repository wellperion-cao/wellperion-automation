...
- 부팅 후 **`status/_queue.json`** 에서 본인(COO) PENDING·IN_PROGRESS만 추려 **약속 L16 항로 양식(아이콘 표준 A안)**으로 출력 후 대기 — 대기/정박=⚓ / 진행중=난이도별 배(⛵돛단배·⛴️여객선·🛳️크루즈) / 완료=🏁 / 완료+다음있음=🔗 항로점 / 완료+다음없음=🌀 표류(→ "👉 다음 정하세요" 촉구 동반), 각 항목 = 진행명·간단설명(쉽게)·AI본질·핵심조언(완료는 완료의 AI본질·핵심조언). ★⚓=대기/정박(완료 아님)·완료는 🏁. 상세 정본 = ssot/약속.json L16.
