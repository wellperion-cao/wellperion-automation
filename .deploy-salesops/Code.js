...
var ALLOWED_CELLS = ['P20', 'I16', 'I18', 'I20', 'I21'];   // I20·I21 = 서버 자동 생성 문안(배1086·1097 · GM 결재 2026-09-07)
...
// ★대상 파일 — file 파라미터가 오면 그 파일을 연다(2026-09-02 GM 지적으로 추가).
//   이 프로젝트는 8월 매출 시트에 붙어 있다. 매달 새 시트를 복사해 만들면 코드는 따라오지만
//   웹앱 주소는 안 따라와서, 9월부터 쓰기가 8월 시트로 계속 들어갔다(09:30 보고는 9월 시트를 읽는다).
//   그래서 '어느 파일에 쓸지'를 부르는 쪽이 정하게 한다 — 부르는 쪽은 이미 매달 시트를 자동으로 찾는다.
//   file 이 없으면 종전대로 붙어 있는 시트를 쓴다(회귀 0).
function _ss(file) {
  return file ? SpreadsheetApp.openById(String(file)) : SpreadsheetApp.getActiveSpreadsheet();
}

function _sheet(file) {
  var sh = _ss(file).getSheetByName(SHEET_NAME);
...
function doGet(e) {
  ...
  if (p.find) {
    var rng = _sheet(p.file).getRange(p.range || 'A1:S40');
  ...
  if (p.lesson) {
    var ss = _ss(p.file);
    var dayName = String(_sheet(p.file).getRange('I1').getDisplayValue() || '').trim();
  ...
  var v = _sheet(p.file).getRange(cell).getDisplayValue();
...
function doPost(e) {
  ...
  var sh = _sheet(body.file);
  // 이번 달 파일이 아니면 쓰지 않는다(2026-09-07 시토 · 8월 시트에 I20·I21 오기입 사고 — 붙어 있는 파일이
  // 달이 바뀌어도 옛 달이라 file 없이 부르면 옛 시트에 쓴다). 파일 이름 = 'yyyy년 M월 매출 보고'.
  var wantName = Utilities.formatDate(new Date(), 'Asia/Seoul', 'yyyy년 M월') + ' 매출 보고';
  var gotName = sh.getParent().getName();
  if (gotName !== wantName) return _json({ ok: false, error: 'wrong file — 이번 달 파일(' + wantName + ')이 아니라 ' + gotName + ' 입니다', ssName: gotName });
