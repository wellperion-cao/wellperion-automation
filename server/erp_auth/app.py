...
def safe_next(next: str, default: str = "/") -> str:
    ...
    슬래시 없는 /erp/admin 은 /erp/admin/ 으로 보정한다(GM 2026-09-19) — 상대경로 CSS 가 슬래시 없이는 깨진다.
    이 함수가 next 검증의 유일한 관문이라 여기 한 곳만 고치면 로그인(이메일·구글·네이버·카카오) 전부에 먹는다."""
    if not (next.startswith("/") and not next.startswith("//") and "\\" not in next):
        return default
    path, _, query = next.partition("?")
    if path == "/erp/admin":
        return "/erp/admin/" + (("?" + query) if query else "")
    return next
...
<label>직함<select name=rank required><option value="">선택</option>{rank_opts}</select></label>
...
def account_page(erp_session: Optional[str] = Cookie(default=None), msg: str = "", err: str = ""):
    ...
    rows = [("이름", d["name"] or "—"), ("아이디", d["email"]), ("부서", d["dept"] or "—"), ("직함", d["rank"] or "—"),
...
if __name__ == "__main__":                     # 회사 계정 판별 자가점검: python app.py
    ...
    assert safe_next("/erp/admin") == "/erp/admin/"            # 슬래시 없는 admin 진입 보정(2026-09-19)
    assert safe_next("/erp/admin?x=1") == "/erp/admin/?x=1"
    assert safe_next("/erp/admin/") == "/erp/admin/"
