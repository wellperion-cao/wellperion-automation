...
# 하는 일: counsel/index.html(공통 템플릿) 을 다캠·고척 QA골프·AX 랩스 폴더에 올리고(1_wellperion 은 ERP 루트 자체가 git
...
# AX 랩스(4_axlabs)는 별도 /axlabs/ 라인 없이 info.nginx.conf 의 /info/axlabs/ 하나로 연다(이 스크립트는 info.nginx.conf 를 안 올린다 — 시토가 따로 반영).
set -euo pipefail
...
$S "sudo mkdir -p /srv/www/3_gocheokgolf /srv/www/4_axlabs && sudo chown ec2-user:ec2-user /srv/www/3_gocheokgolf /srv/www/4_axlabs && mkdir -p /srv/www/2_dietcamp/counsel /srv/www/3_gocheokgolf/counsel /srv/www/4_axlabs/counsel"
...
TMP=$(mktemp -d)
for t in 2_dietcamp 3_gocheokgolf 4_axlabs; do
...
$SCP "$TMP/4_axlabs.html" "$HOST:/srv/www/4_axlabs/counsel/index.html"
rm -rf "$TMP"
...
echo "--- 다캠·고척 QA골프·AX 랩스(즉시) ---"
for p in "dietcamp/counsel/" "gocheokgolf/counsel/" "info/axlabs/"; do
...
check_baked "info/axlabs/"         "4_axlabs"   # info.nginx.conf 를 서버에 올린 뒤에만 200 — 그 전엔 404 로 정상
if [ "$MISMATCH" = "1" ]; then
