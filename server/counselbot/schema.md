...
## 정본 3층 + 공통 학습층 (GM 2026-09-05 구조 확정)

| 층 | 무엇 | 누가 채우나 | 어디 |
|---|---|---|---|
| ① 사실층 | 사업자등록증·가격표·시간표·플레이스·사진에서 나오는 것(질문 세례 📄 60문) | 시보 — 자료 받아 옮김 | `tenants/{t}.json` facts·offerings·policies·channels |
| ② 컨셉층 | 상담원 컨셉(프리셋 4종 중 선택 또는 직접 기술) + 금지어(공통 + 업체 추가) | 우리 — 셋업 때 다 한다 | `shared/concept_presets.json` · `shared/guards_common.json` · 프로필 identity.concept_preset / guards |
| ③ 경험층 | 실제 회원·고객 질문으로 채워지는 빈틈(미답 → 아침 1문 → 답 → 정본 승격) | 대표 답 ← 손님 질문 | `tenants/{t}_qa.json` · 서버 faq.json |
| ★ 공통 학습층 | 질문 **유형**과 답 **뼈대**만(사실 값·개인정보 제외) — 한 지점에서 새 유형이 나오면 전 지점이 같이 배운다 | 시보(승격) · 코드(집계) | `shared/question_types.json` |

- 공통 학습층이 **돈 버는 층**이다: 지점이 늘수록 유형 원장이 두꺼워지고, 새 지점은 사실 칸만 채우면 첫날부터 그 두께로 답한다. 요금은 이 층 접근권(월 구독 · 공통 업그레이드 포함)에 붙인다.
- 절대 규칙: 공통 학습층에는 **고객 개인정보·대화 원문·업체 고유 사실 값**을 넣지 않는다(`never_here`). 이것이 깨지면 사업이 끝난다(§7 위험).
- 코드 배선(시토): 프롬프트 공통부가 `concept_presets`(선택된 것)·`guards_common`(전부)·`question_types`(전부)를 읽는다. `_forbidden_hit` 는 공통 phrases + 업체 phrases 합집합.
