  ...
  if (action === 'member_active_list') {
    ...
    try {
      _cachePutJson_(aaCache, 'aacache|' + aaScope + '|' + (aaWantRows ? 'obj' : 'rows'),
                     aaWantRows ? aaObjResult : aaRowsResult, 330);
    } catch (aaE) {}
    // ★2026-08-03 시토(배298) — TTL 60 → 360초, ★2026-08-29 시토(배11063) — 360 → 330초.
    //   워머(warmLessonRosterCache, 아래 _WARM_EVERY_MIN)가 5분(300초)마다 nocache:'1' 로 다시
    //   읽어 캐시에 넣는다 — TTL 이 워머 주기보다 길면 그 사이에 만료되는 순간이 없어 **항상 히트**한다.
    //   360초는 그 여유를 60초 더 뒀던 것인데, 시트를 직접 손으로 고쳤을 때(2026-08-28 GM 지적
    //   "종료일자만 고쳐도 반영되게 해줄 수 없어?" — 판정 자체는 이미 라이브 반영·잔여일이 종료일자
    //   기준 매번 새로 계산됨을 실측 확인했고, 체감 지연의 실제 원인은 이 캐시다)의 최대 지연 상한이
    //   곧 TTL 값이라 그 60초가 고스란히 낭비였다. 330초로 낮춘다 — 워머 주기(300초)보다 여전히 길어
    //   정상 동작(워머가 살아있는 한 캐시미스 0)은 그대로 지키면서, 직접 편집 시 최대 지연을
    //   6분→5.5분으로 줄인다. 워머 주기(300초) 밑으로는 낮추지 않는다 — 그 밑이면 워머 직후 매번
    //   한 틱은 만료돼 미스가 나고, 그 요청이 콜드 리드(12초, 위 실측 2026-08-05)를 그대로 떠안는다.
    //   ▸직접 편집 지연을 5분 미만으로 더 줄이려면 TTL 이 아니라 워머 주기(현재 5분)를 낮춰야 하는데
    //     이건 이번 티켓 범위 밖이다(onEdit 트리거 신설은 별도 판단 필요 — 약속 L21).
    _cachePutJson_(aaCache, aaCacheKey, aaResult, 330);
