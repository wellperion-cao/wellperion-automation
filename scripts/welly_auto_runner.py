...
def build_orchestration_prompt(ship: dict, clevel: str = "cto", nick: str = "시토") -> str:
    ...
    return (
        ...
        "3) 커밋은 반드시 `python scripts/safe_commit.py -m \"<메시지>\" -- <네가 만든/바꾼 "
        "파일 경로만>` 로 한다(맨손 git add/commit·git add -A·git add .·git commit -a는 "
        "절대 금지 — 공유 라이브 인덱스로 커밋하면 워킹트리에 이미 있던 무관한 미커밋 작업, "
        "예: 다른 세션이 남긴 chro/cfo 인사·재무 파일 삭제까지 네 커밋에 쓸어담는다. "
        "2026-08-04 실사고: COO·CPO 세션이 인사 지원자 사진을 이 방식으로 반복 삭제했다). "
        "safe_commit.py는 네가 지정한 경로 밖은 절대 담지 않고, 무관 경로가 섞이면 커밋 "
        "자체를 만들지 않는다 — [FAIL]이 뜨면 경로를 좁혀 재시도하라. 커밋 메시지 마지막 두 줄:\n"
