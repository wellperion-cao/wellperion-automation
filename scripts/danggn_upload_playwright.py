...
# 당근 비즈 로그인 세션 · storage_state(쿠키·localStorage) — 프로필 손상 회피용
PERSISTENT_PROFILE_DIR, COOKIE_STATE_PATH = profile_paths("danggn")
...
def telegram_report(message: str) -> None:
    ...
    try:
        ok = _tg_send(token, TELEGRAM_CHAT_ID, message,
                       source="danggn_upload_playwright.telegram_report",
...
async def _launch_context(async_playwright):
    ...
    context = await _launch_persistent_with_heal(p, headless=False, args=[*quiet_args()])
