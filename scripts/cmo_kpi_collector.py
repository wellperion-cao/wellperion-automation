...
[수동 실행 전용] 사람이 볼 때 직접 돌리는 조회 리포트 도구 — 무인 배선 대상 아님.
  근거: 산출물(status/cmo_kpi.json · status/cmo_kpi_report.html)을 읽는 코드가 저장소에 0건이고
  (2026-07-23 실측), 산출물 자체도 현재 미존재 — 화면·알림 어디에도 물려 있지 않다.
  즉 주기 실행해도 아무도 안 보는 파일만 생긴다. 필요할 때 사람이 돌려 HTML 을 열어 본다.
  ※ 자동 대시보드는 M1(마케팅 현황) 계열이 담당. 이 도구의 존치/폐기 판단은 시모·GM 몫.
