...
def 화면들() -> list[str]:
    나온것 = []
    for p in glob.glob(f"{GUIDE}/**/*.html", recursive=True):
        정규 = p.replace("\\", "/")
...
def 통과(r: dict) -> bool:
    """다섯 가지를 다 채워야 「지킨다」다. 하나라도 빠지면 안 지키는 것이다."""
...
def 세기() -> dict:
    ...
    return {
        "_이 파일은": "화면 UI 표준의 규격과 지금 지킴 현황. scripts/ui_standard_check.py 가 만든다 — 손으로 고치지 마라. 규격 정본은 assets/wp-ui.css(간격·글자)와 erp/brand/tenant-wellperion.css(색)이고, 이 파일은 그것을 얼마나 지키는지 센 결과다.",
...
def 자가점검() -> None:
    ...
    print("자가점검 OK — 링크·글자·색·모바일 폭·좌측정렬 다섯을 각각 세고, 주석은 안 센다")
