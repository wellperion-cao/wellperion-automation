...
def build_reception_block(target_date: str) -> str:
    ...
    # 분실물은 별도 주기로 도는 건이라 아침 접수 통(중간관리자)·저녁 점검 통은 뺀다. 여기만 포함해
    # 세면 통마다 숫자가 달라 보인다(2026-09-03 실측: 35 / 8 / 13 / 36) — 몇 건이 분실물인지 적는다.
    _lost = sum(1 for r in unresolved if "분실물" in str(r.get("category") or ""))
    lines.append(
        f" • 미해결 {len(unresolved)}건(분실물 {_lost}건 포함) — 2주내 {len(active)} · 보류(2주+) {len(stale)}"
...
✅ 확인된 것 (해당할 때만 · 없으면 이 섹션 통째로 생략)
 • [확인자] [시각] 확인 — 무엇이 확인·완료됐는지 한 줄. 최대 4줄. (이름 뒤에 '님'을 쓰지 마세요 — 발신 관문이 붙입니다. 붙이면 '님 님'이 됩니다)
...
- 답이 있으면 '⚠️ 오늘 챙길 것'에 넣지 않는다. 대신 '✅ 확인된 것'에 **누가·몇 시에 확인했는지와 함께** 적는다.
  예: ' • 최준용M 21:29 확인 — 8/12 기준 멤버십·준회원·유소년 이관 완료'
...
_SCHEDULE_STOP_WORDS = frozenset(
    ...
    " 요청 준비 점검 방문 연락 일정 수업 회원 팀 부 실장 소장 업체 문자 자동"
    " 면접 채용 청소 대청소 골프팀장 교체 발송 마감 오픈 팝업 API 연동 시스템 서버 단말기 결제".split()  # 2026-09-07 웰리: 편도민↔김태엽(다른 지원자·다른 시각)이 '골프팀장+면접'으로 오탐
)
