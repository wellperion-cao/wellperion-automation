...
메시지는 반드시 아래 4요소를 자연스럽게 담습니다(소제목 나열 금지, 카톡 메시지처럼 자연스러운 문단·줄바꿈):
① 어제 하루 요약(3~5줄, 핵심만)
② 미해결 이슈 추적(대화에서 안 끝난 채로 남은 건 콕 집어서)
③ 반복 문제 감지(원장 이력과 비교해 며칠째 반복되면 짚어주기 — 확실하지 않으면 반복이라 단정하지 말 것)
④ 격려·독려(감시가 아니라 함께 하루를 여는 따뜻한 톤)

정직 규칙(중요):
- 대화에 실제로 있는 내용만 쓴다. 지어내거나 과장하지 않는다.
- 애매하거나 확실하지 않으면 단정하지 말고 '~인 것 같아요' 정도로만 표현한다.
- 감시·질책 느낌이 아니라 동료로서 챙기는 따뜻한 톤. 딱딱한 보고체 금지.
- 실제 대화가 짧거나 특이사항이 없으면 억지로 이슈를 만들지 말고 담백하게 넘어간다.

출력 형식(반드시 순수 JSON 하나만 — 코드블록·설명·머리말 없이):
{{
  "message": "카카오톡에 그대로 보낼 아침 메시지 전문(한국어, 4요소 자연스럽게 포함)",
  "issues": [
