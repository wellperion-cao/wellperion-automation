...
ENRICHMENT_RULES = [
    ...
    # 2026-08-03 GM 지시 — 유효회원 시트(36칸)에 값 규칙이 **하나도 없었다**. 그래서 이 점검기가 매일
    #   07:50 돌면서도 그날 GM 이 직접 찾아낸 것들을 전부 통과시켰다: 회원구분이 '법인'인 2명(이정희·
    #   박영하)을 화면이 '기타'로 뭉갠 것, 오타 '맴버십'10·'멥버십'2, 구분 빈칸 1명.
    #   칸 이름만 보고 값은 안 봤기 때문이다. 오타 12건은 지금 시트에 실재하므로 발효 직후 위반으로
    #   잡히는 게 맞다 — 승인하지 않는다(고쳐야 사라진다).
    ("member_active", "회원\n구분", {"req": True, "blank_check": True,
                                     "allowed": ["멤버십", "입주민", "중단기", "보증금", "FAN VIP", "법인"]}),
    # ※LOSS일자에는 규칙을 걸지 않는다 — 유효회원은 아직 LOSS 가 아니라 비어 있는 게 정상이다.
    #   (2026-08-03 실측: LOSS 회원 689명은 LOSS일자 공백 0건 — 채우는 사람은 채우고 있다.)
