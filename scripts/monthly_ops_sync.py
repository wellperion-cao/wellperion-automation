...
def run(month: str | None, apply: bool) -> None:
    ...
    # 전사일정 라이브 → 로컬 시드 단방향 내려받기 (배621·625 · 2026-08-14 시토).
    #   왜 여기인가: 매일 07:00 에 도는 유일한 전사일정 관련 작업이고, 바로 위에서 목표를
    #   라이브로 밀어 넣은 직후라 순서(밀고 → 내려받고)가 자연스럽다. 새 예약작업 0(약속 L21).
    #   왜 필요한가: 사람은 화면(GAS)에 쓰는데 점검 경보·아침 보고는 로컬 파일을 읽어 두 벌이
