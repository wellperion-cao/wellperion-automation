...
가르는 축 = **「누가 움직여야 하나」** (2026-08-04 시토 판정 — GM 2026-07-24 기준과
2026-08-04 지시 "시토 진행건은 AI방"을 합친 것. 두 기준은 충돌하지 않는다):
  AI 가 알아서 하는 일의 **보고·경보**      → tech_check(AI 진행현황방)
            예) 헬스체크 경보, 자동 push 실패, 연동 다리 끊김, 검수·점검 결과,
                발송 실패 배선 경보(내용이 업무보고방 소관이어도 경보는 여기)
  **GM 이 손을 대야 끝나는 요청**            → gm_action(업무보고방)
            예) 세션 열기(러너 모호 배 인터뷰), 구글 재로그인, 봇 토큰 재발급, 결정·승인
  ★'내용이 AI 살림인가'로 가르지 않는다 — 그러면 GM 이 움직여야 할 것이
    GM 안 보는 방으로 가서 며칠 묵는다.
