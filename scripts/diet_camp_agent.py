...
- 순서: 못 받은 것부터 → 그다음은 ①②③④ 를 하루씩 돌아가며(이미 나온 주제는 고르지 않는다).
  ...
  · 못 받은 것(이 순서로): 브랜드가이드 v0.2 색값·표지 사진 확인 1문(사진에서 뽑은 그린·블랙·화이트 색값이 맞는지 · 초안 링크로 확인 요청) · 센터 사진 중 간판·그룹 수업·1:1 재활 장면(폰 사진 그대로 카톡으로 보내 주시면 된다고 안내) · 인스타·블로그 계정 · 글 검수는 누가 · 로고 파일
