...
#                 (미집계가 아니라 "아직 등록자가 없음" — 꼬리표에서 뺀다)
#   · 뮤지컬     : 유소년 문의 146건 · 그중 등록(SUC) 14건 → 그런데 LESSON_DISPLAY 에
#                 항목 자체가 없어 집계 밖. 이게 진짜 구멍이다(꼬리표 유지).
#
# ★남은 구조 구멍(발레·바레·뮤지컬 공통): ledger:true 종목은 원장을 읽지만, 원장에 쓰는
#   쪽(_syncLessonRegistry_)은 팀시트 roster 만 본다 → 팀시트가 없는 이 종목들은 원장에
#   행이 생길 경로가 아예 없다. 읽기만 배선되고 쓰기가 안 된 상태. 발레·바레가 지금 0인
#   것은 우연히 실제로도 0이라 들키지 않았을 뿐, 등록자가 생겨도 0으로 남는다.
#   → 해소는 배140. 그때까지 뮤지컬은 아래 목록에 남겨 정직 표기한다.
LESSON_EXTERNAL_UNTRACKED = ["뮤지컬"]
...
def _fmt_registration_detail_md(mr: dict | None, lr: dict | None, date_from: str, date_to: str) -> str:
    ...
    # [배140 · 2026-07-27 실측 재정정] 뮤지컬은 이제 배선돼 0이 아니라 실제 등록수(14)가 잡힌다 —
    # 그런데 그 14가 배포 사고로 오염됐다(중복 3건 + 오늘 날짜 오기재 11건, 실제 고유 등록자는 11명).
    # 팀시트 없어 '집계 밖'이라던 옛 표현은 더 이상 사실이 아니다 — 지금은 '집계는 되는데 오염'.
    # GM 승인 후 수리(A·B·C) 완료 전까지 이 숫자는 신뢰 불가로 유지.
    lines.append(
        f"- ⚠️ {' · '.join(LESSON_EXTERNAL_UNTRACKED)}: 2026-07-27 배선 배포 중 라이브 원장 오염 발생(배140, GM 승인 대기) — "
        "뮤지컬 원장 14행(중복 3건·11건이 기준선 아닌 오늘 날짜로 오기재, 실제 고유 등록자 11명) "
        "신뢰 불가. 수리 전까지 참고만 할 것"
    )
    # 같은 배포 사고의 부수 피해 — 팀시트가 있는 정상 종목인데도 오염된 유일한 케이스.
    lines.append(
        "- ⚠️ 체조&트램폴린: 같은 배포 사고로 371→369(2건 미복구, 팀시트 대조 전) — 신뢰 불가(배140, GM 승인 대기)"
    )
...
def _registration_oneliner(mr: dict | None, lr: dict | None) -> str:
    ...
    # [배140 · 2026-07-27 재정정] 카드 한 줄 꼬리표도 실측에 맞게 — '외부관리·집계 밖' 은 더 이상
    # 사실이 아니다. 지금은 '집계는 되는데 배포 사고로 오염' 이라 뮤지컬·체조 수치가 불신 대상.
    line2 = (f"· ⚠️ ({' · '.join(LESSON_EXTERNAL_UNTRACKED)}·체조=배포사고 오염, GM 승인 대기(배140) "
             "/ 강습원장 96%는 등록일만 미상·인원은 정상)")
    return line1 + "\n" + line2
