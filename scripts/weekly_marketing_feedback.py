...
def _fmt_registration_detail_md(mr: dict | None, lr: dict | None, date_from: str, date_to: str) -> str:
    ...
    lines.append(
        f"- {' · '.join(LESSON_EXTERNAL_UNTRACKED)}: 팀시트가 없어 등록이 원장에 들어올 경로가 없음 "
        "— 문의는 들어오나 등록 수가 0으로 보임(집계 밖). 해소 = 배140"
    )
...
def _registration_oneliner(mr: dict | None, lr: dict | None) -> str:
    ...
    # [배140] 카드 한 줄 꼬리표도 실측에 맞게 — '외부관리·과소' 라는 옛 표현이 오해를 낳았다.
    line2 = (f"· ({' · '.join(LESSON_EXTERNAL_UNTRACKED)}=팀시트 없어 등록 집계 밖 "
