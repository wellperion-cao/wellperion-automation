...
_SSOT_CLOSED = {"완료", "종결", "취소"}


def gm_work_items_by_person(today: "date | None" = None, rows: "list | None" = None) -> dict:
    """업무&결재 SSOT 의 열린 행을 담당자별로 묶는다.

    ★2026-09-17 원천 교체(GM 「업무 SSOT 로 단일화」) — 종전엔 월간운영계획의 GM 카드를 읽었는데
    그 카드 41장이 같은 날 SSOT 행으로 이관돼 두 곳에서 같은 건이 나왔다. 이제 담당자 칸이 정본이고
    카드·중간관리자 원장의 업무는 읽지 않는다(원장은 회신 소통건만).
    ...
    out: dict = {}
    for r in rows if rows is not None else _fetch_todo_rows():
        if not isinstance(r, dict):
            continue
        if str(r.get("상태") or "") in _SSOT_CLOSED:
            continue
        who = str(r.get("담당자") or "").strip()
        if not who or who.startswith("김남욱"):
            continue
        due = _ymd(r.get("종료일"))
        name = str(r.get("업무명") or "").replace("(GM 직접)", "").strip()   # 이관된 행 제목의 카드 표식은 실무진 통에서 뺀다
        if not name:
            continue
        tail = ""
        if due:
            try:
                ...
                tail = f" — {due}"
        # 한 행에 「실장 + 팀원」처럼 여러 이름이 붙어도 라인장 통에는 한 줄만 간다 —
        # 팀원 이름은 그 줄 끝에 모아 붙인다(같은 건이 두 줄로 겹치던 것 · 2026-09-09 실측).
        routed: dict = {}
...
GM_WORK_SHOW_N = 7        # 사람당 통에 싣는 줄 수 — 기한 빠른 순. 나머지는 화면에서 본다
...
def _selfcheck_gm_work_section() -> None:
    ...
    rows = [{"상태": "진행중", "담당자": "최준용M", "업무명": "상가 안내물", "종료일": "2026-09-30"},
            {"상태": "진행중", "담당자": "김남욱GM", "업무명": "GM 몫", "종료일": "2026-09-30"},
            {"상태": "완료", "담당자": "이정헌 소장", "업무명": "끝난 일", "종료일": "2026-09-30"},
            {"상태": "진행중", "담당자": "", "업무명": "담당 빈칸", "종료일": "2026-09-30"}]
    by = gm_work_items_by_person(today=date(2026, 9, 29), rows=rows)
...
def build_relay_message(contacts: dict, prev_items: dict) -> "tuple[list, dict, list]":
    ...
    for x in queue:
        ...
        # (실무진 화면)을 분리하는 것이 이 한 줄의 목적이다.
        # ★끄는 기준은 '회신'이 아니라 '진행'이다 (GM 정정 2026-09-01: "완료가 아니라(꼭 회신이
        # 아니라) 진행이 된건에 대해서가 완료인거겠지?"). 답장이 없어도 그 일이 실제로 굴러갔으면
        # 끝난 것이고, 답장이 와도 일이 안 움직였으면 계속 싣는다. 그래서 판정 근거는 카톡 답장이
        # 아니라 **일이 진행된 흔적**(제출 기록·화면 값 변화·현장 확인)이며, 그것을 본 사람이
        # relay_until_replied 를 false 로 내린다.
...
#
# ★2026-09-03 GM 지시 — 밀린 접수 목록의 수신처를 ★운영+시설+지원+주차(4부서 합본방)에서
#   ★운영부로 좁혔다. 4부서방에 목록을 통째로 뿌리던 방식이 실제로 안 먹었다(실측 2026-09-03
#   발신분: 8건 중 20일 넘은 것 3건·최장 24일, 전날분에는 44일짜리도 있었다). 여럿에게 같이
#   보이면 아무도 자기 것으로 잡지 않는다 — 처리 책임이 모이는 실장 방 한 곳으로 보낸다(약속 L24).
#   ▸부서별 인지는 그대로다: 새 접수가 들어올 때 나가는 「📮 새 접수 N건」 통지는 종전대로
#     4부서방으로 간다(report_stream_2b_reception). 여기서 옮기는 것은 '밀린 목록'뿐이다.
# ══════════════════════════════════════════════════════════════════════════
#   ★같은 날 GM 정정 — "종합접수처 전체는 중간관리자방에만 나가면 좋을 것 같아 // 운영+시설+
#   지원+주차방은 점검들이랑 종합접수처 1개씩 접수되면 즉시발송 되는걸로만". 아침 운영부 방으로
#   보냈던 것을 ★중간관리자 로 옮긴다 — 실장·소장·나우열M 이 같이 있어 부서를 가로지르는 배정이
#   한 번에 끝난다(약속 L24 채널 분리). 4부서방은 새 접수 즉시 통지(report_stream_2b_reception)만.
