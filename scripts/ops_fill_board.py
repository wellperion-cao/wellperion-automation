...
def build_caption(S) -> str:
    ...
    return (
        ...
        "먼저 오래 멈춘 것부터, 그다음 조금만 채우면 끝나는 것을 모았습니다.\n\n"
        f"■ 오래 멈춘 일 {S['rest']}건 (30일 넘게 손 못 댐)\n"
        "   끝났으면 「완료」로, 기다리는 중이면 「보류」에 이유 한 줄만 남겨주세요.\n"
        "   한 줄만 있으면 때가 됐을 때 시스템이 알아서 다시 올려드립니다.\n\n"
        f"■ 칸만 채우면 되는 일 — 마감일 {S['need_sched']}건 · 점수 {S['need_score']}건\n"
