...
★실측 필드(2026-07-21 실호출로 확인 — 추측 아님):
  ...
  카테고리(2026-09-17 개편) = "[1] 마케팅"·"[2] 매출 및 영업"·"[3] 인사&파트너팀"·
...
# 옛 이름 → 새 이름(2026-09-17 카테고리 개편 · 원장에 옛 값이 남아 있어도 미분류로 안 떨어지게)
_CATEGORY_ALIAS = {
    "[1] 매출 및 영업": "[2] 매출 및 영업",
    "[2] 인사": "[3] 인사&파트너팀",
    "[3] 파트너팀": "[3] 인사&파트너팀",
...
def build_digest(today: str | None = None, channel: str = "telegram") -> list[str]:
    ...
    # ★2026-09-03 웰리 검수 — 09:30 아침 통에 「🌙 하루의 마무리」 제목이 붙어 있었다(8/31 제목
    #   통일 때 저녁 통 이름을 그대로 가져온 것). 이 통은 아침 매출·운영 현황이다.
    header = f"📈 매출·운영 현황 — {today}({weekday}) 09:30"
