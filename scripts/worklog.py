...
# 2026-09-10 GM 지적(「보여주는 구조가 달라지는 것 같은데」) — 종전 문구는 「GM 물음 1개 = 표 1개」만
# 정해 두어, 물음이 없는 턴(레인 완료·진행 중·시스템 알림)에 쓸 형식이 비어 있었다. 그 빈자리를
# 매번 다른 표로 메우다 화면 구조가 흔들렸다. 형식은 8요소 표 하나뿐임을 문구에 박는다.
# 정본 = .claude/skills/wellperion-gm-report/SKILL.md §4-2 「형식은 하나다」.
# ★2026-09-15 GM 지시 「어떤 지시·질문도 모호하면 항상 /deep-interview 하도록 박제」 — 문서(ai-ceo.md·약속 L14)에만
#   있던 규칙이 안 지켜져(같은 날 테크노짐 값 3번 번복·문안 「각 3대 받았다」 지어냄) 매 프롬프트 문구에 박는다.
_GM_REMINDER = ("[형식 고정] 말투=caveman ultra(군더더기·과정 서술 금지) · "
                "GM 화면에 내는 표는 8요소 표 하나뿐(📌GM요청 🔍실측 ✅반영 🔎검수 📤올림 ⏱소요 💡더나았을방법(=GM께 드리는 한 줄·AI 개선은 스스로 적용) 👉GM액션) — "
                "물음이 없는 턴도 같은 표를 쓴다(📌에 무엇 때문에 내는 보고인지) · "
                "중간 보고 금지, 끝나서 검증된 뒤 한 번에 · 보조 표는 그 아래 최대 1장 · "
                "표 밖 줄글은 표로 못 담는 것만 · "
                "★모호하면 착수 전 /deep-interview(AskUserQuestion 카드)로 먼저 좁힌다 — "
                "4요소(대상 화면·범위·공식 용어/값·의도) 중 결과가 갈리는 빈칸이 있으면 추측 착수 금지"
                "(GM 2026-09-15 「모호한데 진행하면 서로 힘들다」)")
