...
# ★2026-08-31 — 아침·저녁 통 이름을 2026-08-29 확정안대로 「🌅 하루의 시작」·「🌙 하루의 마무리」로
#   통일한다(GM 승인 2026-08-31). 제목이 바뀌면 이 목록부터 넓혀야 한다 — 안 그러면 아침 통이
#   실장 경유 가드에 통째로 막힌다(2026-08-19~20 에 실제로 이틀 연속 안 나갔다).
#   옛 머리글도 함께 남긴다: 한 회차라도 옛 제목으로 나가는 동안 막히면 안 된다.
_ROUTINE_DIGEST_HEADS = ("🌅 하루의 시작", "🌙 하루의 마무리",
                         "🌅 어제 ", "📊 [하루 일과 정리]", "[오늘 하루 정리]")
...
# ▸직함이 더 긴 낱말의 꼬리로 들어간 자리는 사람이 아니다 — 「선반장」이 「선반장님」이
#   되어 실무진에게 나갈 뻔했다(2026-09-09 배1162 미리보기에서 잡음). 그래서 붙여 쓴
#   경우는 앞이 낱말 경계에서 시작하는 2~4자 이름일 때만 본다. 띄어 쓴 경우는 종전대로.
# ponytail: 「정리 원장」처럼 띄어 쓴 보통명사가 직함과 같은 낱말이면 아직 걸린다.
#   더 좁히려면 정규식이 아니라 실무진 명부와 대조해야 한다 — 오탐이 실제로 나오면 그때.
# 직함 목록 확대(배 12677 · GM 지시 2026-09-16 "무엇이든 직함 뒤에 님자를 붙여줘") — 대표·회장·고문·차장·매니저 추가.
_TITLES = r"실장|소장|팀장|반장|원장|프로|부장|과장|주임|사원|대리|점장|대표|회장|고문|차장|매니저"
# 이미 존칭이 붙은 자리는 건드리지 않는다 — '님'뿐 아니라 '께'(공손 조사)도 이미 붙은 것으로 본다.
# 사이에 공백이 하나 끼는 실제 표기("GM 께서")까지 잡는다(2026-09-16 실측: 붙여 안 쓴 글이 실제로 있었다).
_ALREADY_HONORIFIC = r"(?!\s?(?:님|께))"
# GM 뒤에 붙는 업무 용어 합성어 — 사람을 가리키는 자리가 아니라 카드 태그·화면 이름·결재 단계 이름이다
# (예: 「GM 지시」 카드 태그, 「GM업무.html」 화면, 「GM 결재」 결재 단계). 띄어 쓴 형태만 문제가 된다 —
# 붙여 쓴 「GM업무」는 뒤에 한글이 바로 오므로 이미 안전하다. 목록은 실측 발신 로그 7일치(배 12677)에서
# 나온 것만 넣었다 — 새 합성어가 오탐으로 나오면 이 목록에 추가한다.
_GM_TAIL_EXCEPT = r"지시|결재|승인|확인|업무|판단|결정|개인|PC"
_HONORIFIC_RES = [
    re.compile(r"(?<=[가-힣])\s(" + _TITLES + r")" + _ALREADY_HONORIFIC + r"(?![가-힣])"),
    re.compile(r"(?<![가-힣])[가-힣]{2,4}(" + _TITLES + r")" + _ALREADY_HONORIFIC + r"(?![가-힣])"),
    re.compile(r"(?<=[가-힣])(AM|M)" + _ALREADY_HONORIFIC + r"(?![A-Za-z가-힣])"),
    # GM 은 합성어 예외가 있어 AM·M 과 따로 뗀다. 붙여 쓴 형태(예: 「김남욱GM」)·띄어 쓴 형태(예: 「김남욱 GM」) 둘 다 본다.
    re.compile(r"(?<=[가-힣])GM" + _ALREADY_HONORIFIC + r"(?![A-Za-z가-힣])(?!\s?(?:" + _GM_TAIL_EXCEPT + r"))"),
    re.compile(r"(?<=[가-힣] )GM" + _ALREADY_HONORIFIC + r"(?![A-Za-z가-힣])(?!\s?(?:" + _GM_TAIL_EXCEPT + r"))"),
]
...
AUTO_PIPELINE_SENDERS = {
    ...
    "대표결재전달",        # rep_approval_relay.py — 대표님 결재 촬영본이 올라오면 그 건을
                          # ★중간관리자 방으로 즉시 전달. GM 승인·상시 지시 2026-09-03:
                          # "이제 항상 결재건은 중간관리자방에 자동 전달해줘 즉시 전달".
                          # [2026-09-03 GM 승인 "추천 진행"] GM 서명 축도 같은 이름으로 나간다(한 통 두 절).
    "업무등록묶음",        # rep_approval_relay.run_new_rows — 업무 SSOT 신규 행 「오늘 올라온 업무」
                          # (★중간관리자 · 12:00·17:05). GM 승인 2026-09-03 "추천 진행" 으로 켬.
}
SENDER_WELLY = "웰리"
...
def _selfcheck_honorifics() -> None:
    cases = [
        ...
        ("실장님 자리", "실장님 자리"),
        # 배 12677(2026-09-16) — 직함 확대(대표·회장·고문·차장·매니저) + GM 합성어 예외 + 께 처리
        ("홍길동 실장 — 확인 부탁드립니다", "홍길동 실장님 — 확인 부탁드립니다"),  # 가짜 이름 기본 케이스
        ("홍길동 실장님 자리", "홍길동 실장님 자리"),                            # 이미 님 — 불변
        ("김가짜M 님께 전달", "김가짜M 님께 전달"),                              # 공백 낀 '님께' — 불변(중복 금지)
        ("이 칸은 대표싸인 칸입니다", "이 칸은 대표싸인 칸입니다"),               # 데이터 키(대표싸인) — 불변
        ("이승기 대표님이 확인했습니다", "이승기 대표님이 확인했습니다"),          # 이미 님 — 불변
        ("양상규 고문께 여쭤보겠습니다", "양상규 고문께 여쭤보겠습니다"),          # 이미 께 — 불변
        ("김철수 차장, 박영희 매니저", "김철수 차장님, 박영희 매니저님"),
        ("오늘 GM 지시 확인 바랍니다", "오늘 GM 지시 확인 바랍니다"),            # GM 합성어(카드 태그) — 불변
        ("이 결재는 GM 결재 단계입니다", "이 결재는 GM 결재 단계입니다"),         # GM 합성어(결재 단계명) — 불변
        ("김남욱 GM 확인 부탁드립니다", "김남욱 GM 확인 부탁드립니다"),           # 이름+GM 뒤 합성어 — 불변(합성어가 우선)
        ("김남욱 GM 승인 뒤 진행합니다", "김남욱 GM 승인 뒤 진행합니다"),         # GM 합성어(승인 단계명) — 불변
        ("이 배는 김남욱 GM 담당입니다", "이 배는 김남욱 GM님 담당입니다"),       # 합성어 아닌 일반 명사는 그대로 님
        ("확인 https://erp.wellperion.com/coo/check/전사_일정.html 보세요",
         "확인 https://erp.wellperion.com/coo/check/전사_일정.html 보세요"),    # URL — 불변
    ]
    for src, want in cases:
