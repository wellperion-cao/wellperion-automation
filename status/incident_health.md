# 🛡️ 재발방지 회귀감시
_갱신: 2026-08-03 · 자동 산출(ssot/incident_regression_monitor.py)_

## 판정: ⚠️ 회귀 감지

| 검사 | 결과 |
|---|---|
| 신규 캐논 발산(baseline 초과) | 7건 · inquiry_path, telegram_chat_id, company_scale, company_address, rule:cta_channel_rules, rule:gm_approval_gate_5 |
| 가드 자산 무결 | OK |
| 박제 무결(GUARDED) | OK |
| 결정정합 누출(leaked · §6.3) | OK(0건) |

### ⚠️ 신규 드리프트(회귀)
- **inquiry_path**: scripts/graphify-out/cache/ast/v0.9.31/5c03e52a8ad498d816c2c320d33cba9f68866bd24448b8a01ded45950db29c26.json, scripts/graphify-out/graph.json
- **telegram_chat_id**: scripts/kakao_report_sender.py
- **company_scale**: status/page_hygiene_proposal_20260802.md
- **company_address**: scratchpad_dump.json
- **rule:cta_channel_rules**: scripts/graphify-out/graph.json
- **rule:gm_approval_gate_5**: scripts/graphify-out/graph.json
...
- telegram_chat_id ← `wellperion-agents/scripts/clevel_post_action.py` (canon 직독으로 전환 권장)
