# 🛡️ 재발방지 회귀감시
_갱신: 2026-08-06 · 자동 산출(ssot/incident_regression_monitor.py)_

## 판정: ⚠️ 회귀 감지

| 검사 | 결과 |
|---|---|
| 신규 캐논 발산(baseline 초과) | 14건 · inquiry_path, telegram_chat_id, bot_token_key, company_scale, rule:cta_channel_rules, rule:gm_approval_gate_5 |
| 가드 자산 무결 | OK |
| 박제 무결(GUARDED) | OK |
...
- **telegram_chat_id**: .deploy-funnel-v2/Survey.js, scripts/kakao_report_sender.py, scripts/module_reporter.py, status/gas_push_baseline.json, tests/test_room_routing.py
- **bot_token_key**: status/gas_push_baseline.json
