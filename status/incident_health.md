...
| 검사 | 결과 |
|---|---|
| 신규 캐논 발산(baseline 초과) | 6건 · telegram_chat_id, company_address |
| 가드 자산 무결 | OK |
| 박제 무결(GUARDED) | 깨짐: INC-010(FIXED(근본원인 정규화·복원 완료 · 견고화는 후속)), INC-011(OPEN(코드 검증 완료 · 라이브 발효=봇 재기동(퇴근 재부팅) 대기)), INC-023(OPEN), INC-024(OPEN), INC-025(OPEN) |

### ⚠️ 신규 드리프트(회귀)
- **telegram_chat_id**: ops/register_cpo_member_expiry_alert.bat, scripts/collectors/cpo_sheet_contract.py, scripts/member_expiry_alert.bat, scripts/reaction_scorecard.py, status/module_registry.json
- **company_address**: status/backups/before_bulk_snapshot_20260720.json
