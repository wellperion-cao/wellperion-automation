 ...
 * 상담실 리뉴얼 · 목욕탕 허가건 진행 · 지원부 약품·비품 비교 구매 구조 만들기) 또는 ②gm_top3_rank 값이 있는
 * 건(GM 이 직접 top3로 지정한 전사 핵심과제, 예: 웰페리온 ERP/브로제이 — 2026-08-07 GM: "GM 업무 화면에
 * 내걸로 넣어놔줘"). 원본 데이터엔 카테고리 칸이 없어 목표 내용을 보고 업무 시트와 동일한 9종 체계로 사람이
...
(function () {
  ...
  var CATEGORY_BY_ID = {
    ...
    // 아래 3건은 제목에 "(GM 직접)" 태그가 없지만 gm_top3_rank(1~3)가 있어 2026-08-07 GM 지시로 함께 편입된다
    // (GM 이 2026-08-01 직접 지정한 top3 전체 — ERP/브로제이 1건만 넣을 이유가 없다, 판정은 순위 셋 다 동일).
    '2026-08-01': '매출 및 영업',   // ①매출&지출 — 월 매출 6.6억 평균 고정
    '2026-08-08': 'IT·시스템·자동화', // ②웰페리온 ERP/브로제이 도입·안정화
    '2026-08-05': '회원·CS'        // ③회원 서비스(회원 서비스 및 시설 밸류업 개편분)
  };
  ...
  // gm_top3_rank(1~3)가 있으면 GM 이 직접 top3로 지정한 전사 핵심과제 — "(GM 직접)" 제목 태그가 없어도
  // GM 업무 화면에 함께 실린다(2026-08-07 GM 지시). 산식 재정렬 금지(monthly_ops_plan.json 값 그대로).
  function isGmTop3(o) { return o && typeof o.gm_top3_rank === 'number'; }
    ...
    return objs
      ...
      .map(function (o) {
        return {
          ...
          // direct=true → "(GM 직접)" 태그 건 / false → gm_top3_rank 로만 편입된 전사 핵심과제(화면에서 배지로 구분).
