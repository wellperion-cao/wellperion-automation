...
var REG_CATEGORIES = [
  ...
  // dept 를 비워 둔다 — 청결은 장소가 담당을 정한다(_regDeptFor). '지원부'(구분 전)는
  // 2026-08-28 GM 지시로 없앴다. 장소를 모르는 건은 빈 값으로 남아 화면에서 사람이 배정한다.
  { key: 'clean',    label: '청결 이슈 접수',       sheet: '접수_청결',     dept: '',      slaHours: 12 },
...
// 장소 → 부서 (GM 확정 2026-08-25 컴플레인 · 2026-08-28 청결까지 확대).
...
//   ▸컴플레인·청결에 적용한다 — 분실물·습득물은 리셉션 보관이라 운영부가 맞고, 시설물 고장은
//     시설부다. 장소를 전 분류에 적용하면 분실물이 P.T팀으로 간다.
//   ▸목록에 없는 장소(자유입력·빈칸)면 종전대로 그 분류의 기본 부서로 둔다.
//   ★2026-08-28 GM 확정 — 청결도 이 표를 쓴다. GM 원문: "수영장은 지원부보단 수영팀에서
//     답변을 주는게 맞을 것 같고". 실측(같은 날 미구분 '지원부' 9건): 수영장 4 · 헬스장 1 ·
//     체조장 1 · 여자사우나 1 · 자유입력 2 — 청결도 장소가 답을 아는 사람을 정확히 가리켰다.
//     성별 규칙(여자·남자)이 이 표보다 먼저 적용된다(아래 _regDeptFor).
var REG_LOC_DEPT = {
  '헬스장': 'P.T팀',
  '수영장': '수영팀',
  '남자사우나': '지원부(남)',
  '여자사우나': '지원부(여)',
  // '락커' 는 남/여가 물리적으로 갈리는데 접수 폼 선택지는 한 칸뿐이라 성별을 알 수 없다.
  // 임의로 한쪽 반장님께 보내지 않는다 — 빈 값으로 두면 화면에서 사람이 배정한다(GM 2026-08-28
  // "지원부 삭제" 이후 성별 미상 건이 갈 자리). 실측 당시 락커 접수는 0건.
  '락커': '',
  '골프장': '골프팀',
  '스쿼시장': '스쿼시팀',
  '체조장': '체조팀',
...
// 접수 건의 담당 부서 — 장소가 정한다(GM 지시 2026-08-15 남/여 · 2026-08-25 컴플레인 · 2026-08-28 청결).
// 순서에 뜻이 있다:
//   ① 장소 글자에 남/여가 있으면 그 구역 반장님 (여자락커·여자사우나 같은 자유입력까지 잡는다)
//   ② 없으면 장소 표(REG_LOC_DEPT) — 수영장=수영팀, 헬스장=P.T팀 …
//   ③ 표에도 없으면 그 분류의 기본 부서
// ★'지원부'(구분 전)는 2026-08-28 GM 지시로 없앴다. GM 원문: "지원부 / 지원부(남) / 지원부(여)
//   이렇게 있던데 지원부 <삭제". 성별을 알 수 없는 건은 빈 값으로 두어 화면에서 사람이 배정한다 —
//   임의로 한쪽 반장님께 찍어 보내지 않는다(실측 2026-08-17: 청결 14건 중 장소가 성별을 밝힌 건은
//   여자사우나 3건뿐이었다).
function _regDeptFor(cat, loc) {
  if (!cat) return '';
  var s = String(loc || '').trim();
  if (cat.key === 'complaint' || cat.key === 'clean') {
    if (/여자|여성/.test(s)) return '지원부(여)';
    if (/남자|남성/.test(s)) return '지원부(남)';
    // '기타' 는 컴플레인이면 운영부가 맞지만, 청결이면 어느 구역인지 모른다는 뜻이라 사람이 배정한다.
    var d = (cat.key === 'clean' && s === '기타') ? '' : REG_LOC_DEPT[s];
    return (d === undefined) ? cat.dept : d;   // '' 는 '사람이 배정' — 기본 부서로 되돌리지 않는다
  }
  return cat.dept;
}
