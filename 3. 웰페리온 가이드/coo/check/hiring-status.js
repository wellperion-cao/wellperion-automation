/* ══════════════════════════════════════════════════════════════════════════
   ...
   ⚠️ 부서 개수 정정(2026-07-20) — 실제 부서는 4개(주차관리부·운영부·시설부·지원부)뿐이다.
      ...
      골프 프로(golfpro.html)는 4개 부서에는 안 붙지만 파트너팀 체계 페이지가 이미
      dept:'파트너팀'로 쓰고 있다 — dept:null로 바꾸면 그 페이지 표시가 사라지는
      회귀가 생긴다(실제로 한 번 발생·복구함). '파트너팀'은 4개 부서 이름 목록에
      없어 4개 부서 탭에는 새지 않는다.
      진짜로 쓰는 곳이 없는 공고(수행기사)만 JOB_PAGES에 dept:null로 남긴다 —
      억지로 아무 데나 끼워 넣지 않고, 대응표에서 눈에 보이게 둔다.
...
(function () {
  ...
  var JOB_PAGES = [
    ...
    // 시설부 — 2026-07-20 현재 시설부로 분류되는 공고 없음(모집 없음으로 표시됨).
    // golfpro.html은 4개 부서(주차관리부·운영부·시설부·지원부)에는 안 붙지만 파트너팀
    // 체계.html이 이미 dept:'파트너팀'로 이 컴포넌트를 쓰고 있다 — '부서 미해당'으로
    // dept:null 처리하면 파트너팀 페이지의 기존 표시가 사라지는 회귀가 생긴다(2026-07-20
    // 실측·수정). '파트너팀'은 4개 부서 목록에 없는 별개 값이라 4개 부서 탭에는 새지
    // 않는다 — 부서 페이지가 자기 dept 이름으로만 필터링하기 때문.
    { file: 'golfpro.html',    dept: '파트너팀', bucket: null,     jobkey: '오전 골프 프로' },
    // chauffeur.html(수행기사)은 실제로 쓰는 페이지가 없다(index.html 채용 목록에도
    // 아직 미게시) — 진짜 미해당이라 dept:null 유지.
