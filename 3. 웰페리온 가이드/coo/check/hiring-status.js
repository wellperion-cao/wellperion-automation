/* ══════════════════════════════════════════════════════════════════════════
   ...
   ⚠️ 범위 확정(2026-07-20) — GM: "4개 부서 각각 자기 부서 채용 공고만 가져오라고
      했던 것"이다. 대상은 4개 부서(주차관리부·운영부·시설부·지원부)뿐이고, 파트너팀은
      범위 밖이다(파트너팀 체계.html에 있던 채용 연동은 범위 초과였음이 확인돼 제거함
      — 원상복구). 이 컴포넌트를 쓰는 페이지는 이제 4개 부서 체계 페이지뿐이다.
...
(function () {
  ...
  var JOB_PAGES = [
    ...
    // 명시적으로 '부서 미해당'으로 남긴다(dept:null → 어느 부서 탭에도 뜨지 않음).
    // golfpro.html(오전 골프 프로)은 파트너팀 성격 — 파트너팀 체계.html은 범위 밖이라
    // 이 컴포넌트를 쓰지 않으므로(2026-07-20 배선 제거) dept:null이어도 회귀 없음.
    { file: 'golfpro.html',    dept: null, note: '파트너팀 성격(오전 골프 프로) — 4개 부서 아님. 파트너팀 체계.html은 이 컴포넌트를 쓰지 않음(범위 밖, 배선 제거됨).', bucket: null, jobkey: '오전 골프 프로' },
    // chauffeur.html(수행기사)은 index.html 채용 목록에 아직 게시되지 않은 페이지 —
    // 현재 어느 부서 체계에도 연결하지 않는다(의도적 제외 · 누락 아님).
    { file: 'chauffeur.html',  dept: null, note: '수행기사 — 4개 부서 어디에도 속하지 않음. index.html 채용 목록에도 아직 미게시.', bucket: '수행기사', jobkey: null }
  ];
