# Wellperion Summer Special Program Inquiry Form (English)

**Form Title:** Wellperion Private Sports Club — Summer Special Programs
**Form Description:** Make the most of your summer at Wellperion. Our Summer Special programs offer intensive sessions across five premium sports disciplines. Submit your inquiry and our team will reach out with program details and scheduling options.

> **Language Integration Note:** Add a hidden field `Language` with fixed value `EN` so all responses merge into the existing Korean response sheet under a unified language-distinguishing column.

---

## Fields

### 1. Full Name
- **Field type:** Short answer
- **Required:** Yes
- **Label:** Full Name
- **Placeholder:** e.g., Kim Namwook

---

### 2. Mobile Phone Number
- **Field type:** Short answer
- **Required:** Yes
- **Label:** Mobile Phone Number
- **Placeholder:** e.g., 010-0000-0000

---

### 3. Participant Type
- **Field type:** Multiple choice (single selection)
- **Required:** Yes
- **Label:** Participant Type
- **Options:**
  - Adult (18 and above)
  - Youth / Child (under 18) — guardian inquiry

---

### 4. Summer Special Program of Interest *(select all that apply)*
- **Field type:** Checkbox (multiple selection)
- **Required:** Yes
- **Label:** Summer Special Program of Interest
- **Options:**
  - Summer Swimming Intensive
  - Summer Gymnastics Intensive
  - Summer Squash Intensive
  - Summer Golf Intensive
  - Youth Summer Personal Training (Growth-Focused PT)

---

### 5. Preferred Program Period
- **Field type:** Multiple choice (single selection)
- **Required:** No
- **Label:** Preferred Program Period
- **Options:**
  - July 2026
  - August 2026
  - Either / Flexible

---

### 6. Preferred Session Time
- **Field type:** Checkbox (multiple selection)
- **Required:** No
- **Label:** Preferred Session Time *(select all that apply)*
- **Options:**
  - Early morning (before 8 AM)
  - Morning (8 AM – 12 PM)
  - Afternoon (12 PM – 5 PM)
  - Evening (5 PM – 9 PM)

---

### 7. How Did You Hear About Us?
- **Field type:** Multiple choice (single selection)
- **Required:** Yes
- **Label:** How Did You Hear About Us?
- **Options:**
  - Member referral
  - Instagram / Social media
  - Online search
  - Building / Residential community notice
  - Other

---

### 8. Additional Requests or Comments
- **Field type:** Short answer (paragraph)
- **Required:** No
- **Label:** Additional Requests or Comments
- **Placeholder:** Please share any scheduling needs, physical conditions, or special requests our program team should consider.

---

### 9. Privacy Consent
- **Field type:** Checkbox (single item — must be checked to submit)
- **Required:** Yes
- **Label:** Consent to Collection and Use of Personal Information

**Consent Text:**

> In accordance with the **Personal Information Protection Act (PIPA) of the Republic of Korea**, Wellperion Co., Ltd. ("the Company") collects and processes your personal information as follows.
>
> **Purpose of collection:** Processing summer program inquiries; scheduling consultations and program registration; participant matching and communication.
>
> **Items collected:** Name, mobile phone number, participant type, program interest, preferred period and session time, additional requests.
>
> **Retention period:** Retained for 1 year from the date of collection, or until the purpose is fulfilled, whichever is earlier. Upon expiration, all personal data will be destroyed without delay.
>
> **Third-party disclosure:** Your personal information will not be shared with third parties without your separate consent, except as required by applicable law.
>
> For inquiries submitted on behalf of a minor, the legal guardian's consent is required and covers both the guardian's and the child's personal information.
>
> You have the right to refuse consent; however, doing so may prevent us from processing your inquiry.
>
> ☐ **I have read and agree to the above terms for the collection and use of my personal information.**

---

*All Summer Special programs are by prior reservation only. Availability is limited — early inquiry is encouraged.*
*For immediate assistance: http://wellperion.com/ko/inquiry/*
