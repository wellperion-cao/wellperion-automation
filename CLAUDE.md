...
> 💰 **토큰 캐시 2규칙(고정)** — ① **모델 라우팅:** 루틴(read·patch·집계·송부)=Sonnet/Haiku, Opus=판단·검토만 (상세 매트릭스=S2⑨). ② **idle:** 긴 작업 후 자리 비울 땐 `/clear`로 캐시 끊기.

> 📐 **SSOT .md 문서 규칙(누적기록 금지 · 2026-06-12 정립)** — 본 파일 포함 모든 살아있는 SSOT .md = **현행 규칙·파이프라인·상태만** 담는다. 결정 경위·사고 기록·폐기 사유를 본문에 쌓지 말 것. 규칙 변경 = 해당 섹션 **교체(덮어쓰기)**, 경위는 짝 이력 파일에 날짜별 append (본 파일 → `docs/CLAUDE_이력.md` / AI 시리즈 → `instagram/_AI시리즈_이력.md`). 스크립트가 파싱하는 표(예: 로드맵 편별 표)는 구조 변경 금지.
...
## 2. R/R SSOT — 웰페리온 ERP
> **운영 원칙·R/R 단일 출처 = S2 웰페리온 ERP(공통탭 + 본인탭).** CLAUDE.md는 인덱스 — 원칙 상세 하드코딩 금지, 원칙 추가는 S2에만.
...
- ※ CEO 인박스 DB(INB) = 폐기됨 — 호출 금지 (텔레그램 보고+승인 전용화)
- 보고 포맷·운영 원칙 상세 = S2 공통탭 fetch (사이클 보고 3섹션+5필드 등 정본은 S2 단일 출처).
...
- 박제 원리(행동=코드/훅·지식만 메모리·추가는 net-zero) 정본 = S2 공통탭 통합원칙. 반복 지적은 '코드 강제 누락' 신호.

### 토큰 라우팅 매트릭스
> 라우팅 표 정본 = **S2 공통탭 ▸ 전 C-Level 통합원칙 ⑨ 토큰 라우팅 매트릭스** (Haiku=단순 read·lookup·송부 / Sonnet=가동·patch·집계·git·콘텐츠·로그 / Opus=판단·결정·검토·결재·이슈진단). 상세·갱신은 S2에만 — 표 중복 금지.
