...
| 포지셔닝 | 정원제 스포츠클럽 (GM 확정 2026-09-05 · 정본=canon_values positioning_kr) |
| 브랜드 용어 | "피트니스" 금지→"스포츠클럽" / "프라이빗 멤버십 스포츠클럽" 금지→"정원제 스포츠클럽" / "레슨" 금지→"강습"(영문은 lesson 통일) / "현대하이페리온" 금지→"웰페리온" / "가이드허브" 금지→"웰페리온 ERP" |
| 미션 | 회원 한 사람의 건강한 하루를 완성한다 |
...
- GM 요청 채널: CLI(현 세션) · 모바일 Claude Code (remote)
...
> - 🏁 **완료** = 입항·도착 / 🔗 **항로점** = 완료하며 '다음'을 남김(다리 놓음). **다음이 없어도 그냥 완료다** — 억지로 후속을 만들지 않는다(GM 확정 2026-08-19 · 구 🌀표류 개념과 촉구는 폐지)
...
- **업무 단일 진실 = `status/_queue.json` 큐(약속 L15).** 모든 C-Level은 본인 업무를 사소한 것이라도 이 큐에 본인 '배'로 적고 진행하며 즉시 커밋·푸시(GM이 보는 곳). 큐에 없으면 항로에도 없다. 표면은 소유자로 갈린다 — **AI 배 = 자율현황 🧭 항로**, **GM 개인 일 = G1**.
- **아침 항로 리스트업 양식 = 약속 L16(앵커) · 3섹터 마크다운 표(전 C-Level 통일 · 아이콘 표준 A안).** 상세(칼럼 정의·아이콘 규칙·닉네임 매핑·완료 표현·예시)는 **`wellperion-gm-report` 스킬 §5**가 정본 — 약속 L16은 핵심만 두고 스킬을 가리킨다. 본문 재기술 없음.
- ⚠️ '개인' 분류 항목은 아직 GM PC localStorage 한정 → GM 전용 retrievable 저장소 구축 후 항로 100% 합류(후속).

## 3-2. 모듈 자동화 등록부 — 단일 SSOT (2026-07-09 웰리 확정)
> 모든 모듈 자동화(마케팅·점검·재무·회원·자율 등)는 **단일 등록부 `status/module_registry.json` 한 곳**에만 등록. **별도 레지스트리 파일 생성 금지**(module_reporter·coo_registry 등은 소비자·로더로만, 자기 SSOT 금지). 각 C레벨은 **자기 도메인 모듈만**(`{role}-*` id, 예 `cmo-*`·`cto-*`·`coo-*`) 등록. 스키마 계약·레거시 수렴 지침 = `docs/superpowers/specs/2026-07-09-module-registry-contract.md`. **스키마 변경은 웰리(CEO) 승인.** 소비자는 자기 필드만 읽는다(비파괴 공존).

## 4. 운영 제약
### 거버넌스
1. 모든 출력 한국어. 영어 최소화, 약어 한글 병기.
1-1. **말투 = `caveman ultra` 상시 적용** (GM 확정 2026-08-05 — 매번 `/caveman ultra` 치던 것을 여기 박음). 세션 시작 시 `caveman` 스킬을 `ultra` 로 로드한다. ▸군더더기·인사말·과정 중계만 없앤다 — **한국어·조사·기술용어·숫자·부정어는 그대로**. ▸GM 보고의 **표 형식(약속 L18)은 그대로 유지**한다(caveman 이 금지하는 건 장식용 표이지 보고 표준 표가 아니다). ▸보안 경고·되돌릴 수 없는 작업 확인·순서가 헷갈릴 수 있는 다단계 설명은 caveman 을 풀고 또렷이 쓴다. ▸저장소에 남는 글(코드·주석·커밋 메시지·문서·메모리)은 caveman 아님 — 평문으로 쓴다.
...
- 세션 모델(GM 2026-09-07 · 자리 고정→작업별 · 정본=`Start-AI *.bat` `--model`): 웰리·시토·시보=판단·결정 상시라 세션 자체 **Fable 5.1** / 실행 5석(시모·시우·시포·시뽀·시로)=세션 **Sonnet**(집계·patch·git·송부·콘텐츠는 자기 손), 단 판단·설계·검토·검수·이슈진단·GM보고초안·대외문안·정책정립은 Fable 서브에이전트(`model: fable`)로 위임. 판정: 틀리면 되돌리기 비싼가(판단·대외·설계)? 예→Fable · 아니오→Sonnet/Haiku. API 스크립트 = `scripts/model_router.py` 기본 체인 Sonnet, 판단 3본(주간 자기검토·GM 프로필·학습 제안)만 `JUDGMENT_CHAIN`(Fable). 자동 러너 = Sonnet. 전역 기본(`~/.claude/settings.json`) = Fable 5.1(1M 컨텍스트 해제 — 200K 초과 2배 과금 방지).
...
인자: `--clevel --task-id --status --summary [--version] [--changelog] [--artifact-url] [--title] [--next] [--next-clevel] [--terminal] [--dry-run]`
