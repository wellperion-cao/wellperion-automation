...
- 모든 C-Level(+CEO)은 작업 완료·대기 시마다 표준 사이클 보고 출력. 포맷 정본 → 가이드허브 g10 공통 탭.
  3섹션 + 🔴 GM 결정, 항목 5필드(식별자 │ 내용 │ 상태 │ 담당자(닉네임) │ 링크):
  `[닉네임(직책) — 날짜]` / ✅ 오늘 완료 / 🟡 진행 중 / ⏳ 내일 할 일 / 🔴 GM 결정 필요(맨 아래 고정, 없으면 "없음" 명시).
  닉네임: 웰리(CEO)·시모(CMO)·시토(CTO)·시우(COO)·시뽀(CFO)·시포(CPO)·시로(CHRO).
