...
- 보고 포맷 정본 = `wellperion-gm-report` 스킬 §4(8요소 표) 한 곳. S2 공통탭은 그 절차를 렌더만 하는 소비자다 — 정의를 두 곳에 두지 않는다(약속 L01). 구 '사이클 보고 3섹션+5필드'는 폐기.
...
- **배(큐) = 체계·시스템화 작업만(약속 L15 · GM 정의 2026-09-16).** 배에 담는다 = 그 일을 체계·시스템으로 굳히겠다는 뜻. AI 가 만드는 구조·자동화·화면·배선·검수기만 `status/_queue.json` 에 본인 '배'로 적고 즉시 저장·배포. **사람이 굴리는 실업무(회신 대기·견적·결재 진행·통화·접수)는 배가 아니라 GM업무 카드·중간관리자 업무·전사일정**에서 추적한다(판정 = 「다음 한 걸음이 사람 손인가?」). 표면 — **AI 배 = 자율현황 🧭 항로**, **GM 개인 일 = G1**.
