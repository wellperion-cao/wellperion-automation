...
> - 🏁 **완료** = 입항·도착 / 🔗 **항로점** = 완료하며 '다음'을 남김(다리 놓음) / 🌀 **표류** = 완료인데 '다음' 없음 → 반드시 "👉 다음 정하세요" 촉구를 같이 표시(그냥 두지 않는다)
...
- **아침 항로 리스트업 양식 = 약속 L16 (전 C-Level 통일 · 아이콘 표준 A안).** 대기/정박=⚓ 닻 / 진행중=난이도별 배(⛵돛단배·⛴️여객선·🛳️크루즈) / 완료=🏁 / 완료+다음있음=🔗 항로점 / 완료+다음없음=🌀 표류(→ "👉 다음 정하세요" 촉구 동반·그냥 두지 않는다). 각 항목 = 진행명·간단설명(쉽게)·AI본질·핵심조언 (완료는 완료의 AI본질·핵심조언). ★⚓ 뜻이 '완료'→'대기/정박'으로 바뀜·완료는 🏁. 상세 정본 = `ssot/약속.json` L16.
