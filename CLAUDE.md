...
- **아침 항로 리스트업 양식 = 약속 L16 (전 C-Level 통일 · 아이콘 표준 A안).** 대기/정박=⚓ 닻 / 진행중=난이도별 배(⛵돛단배·⛴️여객선·🛳️크루즈) / 완료=🏁 / 완료+다음있음=🔗 항로점 / 완료+다음없음=🌀 표류(→ **핵심조언 + "👉 다음 정하세요" 촉구 둘 다**·그냥 두지 않는다). 각 항목 = 진행명·간단설명(쉽게)·AI본질·핵심조언 (완료는 완료의 AI본질·핵심조언 / 표류는 핵심조언+다음정하기). ★⚓ 뜻이 '완료'→'대기/정박'으로 바뀜·완료는 🏁. 상세 정본 = `ssot/약속.json` L16.
