...
- **업무 단일 진실 = `status/_queue.json` 큐(약속 L15).** 모든 C-Level은 본인 업무를 사소한 것이라도 이 큐에 본인 '배'로 적고 진행하며 즉시 저장·배포(GM이 보는 곳). 큐에 없으면 항로에도 없다. 표면은 소유자로 갈린다 — **AI 배 = 자율현황 🧭 항로**, **GM 개인 일 = G1**.
