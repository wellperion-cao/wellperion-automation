...
분류 로직 (Claude API 호출 없음, 순수 Python):
  ...
  3. 지시/질문 (기본값) → "GM 전달사항 접수" 회신 (담당이 이어서 처리 · GM 확정 2026-07-28)
...
"""
    ...
    # 회신 문구 = 'GM 전달사항'(GM 확정 2026-07-28). 전엔 "다음 CEO 세션에서 처리합니다" 였는데,
    #   봇 privacy 해제로 이 회신이 실무진방에도 뜨게 됐다(2026-07-28 15:37 시설팀 알람방 실측).
    #   직원들이 보는 자리에서 GM 말씀이 'CEO 지시' 로 읽히는 건 실제 관계와 다르다 — GM이 남기신
    #   전달사항이고, 담당이 받아 처리한다. 분류 키(directive)는 다른 곳이 참조하므로 그대로 둔다.
    return ("directive", "GM 전달사항 접수했습니다. 담당이 이어서 처리합니다.")
