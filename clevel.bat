...
cd /d "%BAT_DIR%"
REM ★2026-08-04 근본수리(유령 삭제 근본분석) — 맨손 `git add -A`+`git commit`+`pull --rebase
REM --autostash` 제거. 공유 작업트리에서 남의 미커밋 삭제·변경을 통째로 쓸어담던 문이었고
REM (원격이 추가한 파일은 무접촉 통합이 디스크에 실체화하지 않아 add -A 가 삭제로 커밋),
REM autostash 는 INC-034 뿌리. 커밋은 safe_commit 관문 단일 경유(지정 경로만·push 포함).
REM 대상 = 큐 SSOT 2본(役 json 은 보조 메타 — 대문자 %ROLE% 을 git pathspec 에 못 쓰므로 제외).
python "%BAT_DIR%scripts\safe_commit.py" -m "%COMMIT_MSG%" --holder clevel_bat -- "status/_queue.json" "3. 웰페리온 가이드/status/_queue.json"
